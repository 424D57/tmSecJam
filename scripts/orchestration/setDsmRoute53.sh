...
dsmPublicElbHostedZoneId=$(aws cloudformation describe-stacks --stack-name ${stackname} --query 'Stacks[0].Outputs[?OutputKey==`ELBCanonicalHostedZoneNameID`].OutputValue' --output text)
